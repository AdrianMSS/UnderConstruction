...
;(function(window, document, undefined) {
	...
	var videojs = require('../vendor/video-player/video.js');
	...
	var bodyClasses = ['open-nav', 'open-language', 'open-share'];
	...
	// Menu SVG animation
	var shapeElement = document.querySelector( '.navigation-overlay .svg-background' );
	var s = Snap( shapeElement.querySelector( 'svg' ) );
	var pathElement = s.select( 'path' );
	var paths = {
		reset : pathElement.attr( 'd' ),
		open : shapeElement.getAttribute( 'data-morph-open' ),
		close : shapeElement.getAttribute( 'data-morph-close' )
	};
	...
	$('.js-toggle-share').on('click', function() {
		switchBodyClass('open-share');
	});

	$('.js-toggle-language').on('click', function() {
		switchBodyClass('open-language');
	});
	...
	var $paralaxItem = $jsParallax.find('.intro-inner');

	function introParallax() {
		initParallax(scroll, $jsParallax, $paralaxItem, 4, true);
	}
	...
	$win.scroll(function (event) {
		...
		if($jsParallax.length) window.requestAnimationFrame(introParallax);
		window.requestAnimationFrame(checkBodyScroll);
	});

	//clock function
	var locationClock = $('.fn-clock');

	if(locationClock.length) {
		var dateObj = getTime(),
		oldSecond, oldMinute, oldLondonHour, oldAmsterdamHour;

		setInterval(function() {
			oldSecond = dateObj.equalTime.seconds,
			oldMinute = dateObj.equalTime.minutes,
			oldLondonHour = dateObj.londonHour,
			oldAmsterdamHour = dateObj.amsterdamHour;
			dateObj = getTime();
			//remove old time
			$('.clock-hand-seconds').removeClass('seconds-' + oldSecond);
			if(dateObj.minutes != oldMinute) $('.clock-hand-minute').removeClass('minutes-' + oldMinute);
			if(dateObj.londonHour != oldLondonHour) $('.location-london > .clock-hand-hour').removeClass('time-' + oldLondonHour);
			if(dateObj.amsterdamHour != oldAmsterdamHour) $('.location-amsterdam > .clock-hand-hour').removeClass('time-' + oldAmsterdamHour);

			//add new time
			$('.clock-hand-seconds').addClass('seconds-' + dateObj.equalTime.seconds);
			$('.clock-hand-minute').addClass('minutes-' + dateObj.equalTime.minutes);
			$('.location-enschede > .clock-hand-hour').addClass('time-' + dateObj.amsterdamHour);
			$('.location-london > .clock-hand-hour').addClass('time-' + dateObj.londonHour);
		}, 1000);
	}

	function initParallax(scroll, parent, parallaxItem, multiplier, opacity) {
		if($win.width() > 1025) {
			var parentHeight = parent.height(),
				itemHeight = parallaxItem.height(),
				realScroll = scroll/(itemHeight/parentHeight * multiplier),
				opacity = opacity ? 1-(scroll/parentHeight)*1.5 : 1;

			parallaxItem.css({transform: 'translateY(-'+realScroll+'px)', opacity: opacity});
		}
	}
	...
	function getTime() {
		...
		var timeObj = {
			...
			amsterdamHour: currentDate.getHours(),
			londonHour: londonDate.getHours()
		}
		return timeObj;
	}

	// Play Video
	$('.js-play-video-intro').on('click', function() {
	...
	var $playBtn = $('.js-play-video').on('click', function(event) {

		if($('.video-js').hasClass('about-video')) {
			$("html, body").animate({ scrollTop: $('.video-js').offset().top + "px" });
		}
		else {
			introVideo = initVideo($('.js-bert-video')[0], $playBtn, $stopBtn, true);
			$('.video-overlay').addClass('active');
			$('body').addClass('video-play');
		}
	});

	var $stopBtn = $('.js-stop-video').on('click', function(event) {
		$('.video-overlay').removeClass('active');
		$('body').removeClass('video-play');
	});


	$('.js-custom-video-player').each(function () {
		initVideo(this);
	});

	var parallaxJs = $('#intro-parallax');
	if(parallaxJs.length && $win.width() > 1024) {
		var scene = document.getElementById('intro-parallax');
		var parallax = new Parallax(scene);
	}

	function initVideo ($video, customPlayButton, customStopButton, play) {
		...
		var myPlayer 		= videojs(videoElement, {
			...
			if (customStopButton) {
				customStopButton.on('click', function(event) {
					video.currentTime(0);
					video.pause(0);
					video.volume(1);
				});
			...
			if(customPlayButton) {
				customPlayButton.on('click', function(event) {
					video.play();
				});
			}
...
},{"../vendor/video-player/video.js":6,"fastclick":2,"jquery":3,"snapsvg":4}],6:[function(require,module,exports){
